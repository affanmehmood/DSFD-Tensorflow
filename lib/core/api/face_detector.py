...
class FaceDetector:
    def __init__(self, model_path):
        ...
        with self._graph.as_default():
            ...
            self.input_image = tf.compat.v1.get_default_graph().get_tensor_by_name('tower_0/images:0')
            self.training = tf.compat.v1.get_default_graph().get_tensor_by_name('training_flag:0')
            self.output_ops = [
                tf.compat.v1.get_default_graph().get_tensor_by_name('tower_0/boxes:0'),
                tf.compat.v1.get_default_graph().get_tensor_by_name('tower_0/labels:0'),
                tf.compat.v1.get_default_graph().get_tensor_by_name('tower_0/scores:0'),
                tf.compat.v1.get_default_graph().get_tensor_by_name('tower_0/num_detections:0'),
            ]
    ...
    def __call__(self, image, score_threshold=0.5):
        ...
        for i in range(boxes.shape[0]):
            boxes[i] = np.array(
                [boxes[i][1], boxes[i][0], boxes[i][3], boxes[i][2]])  #####the faceboxe produce ymin,xmin,ymax,xmax
